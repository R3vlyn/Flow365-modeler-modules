...
/**
 * Returns 'true' if the given element is 'camunda:Runscript'
 *
 * @param {djs.model.Base} element
 *
 * @return {boolean} a boolean value
 */
ImplementationTypeHelper.isRunScript = function(element) {
  return is(element, 'camunda:Runscript');
};
...
ImplementationTypeHelper.getExternalTopic = function(element) {
...
ImplementationTypeHelper.getRunScript = function(element) {
  var bo = this.getServiceTaskLikeBusinessObject(element);
    
  if (this.getExternalTopic(element) === "runscript") {
    return bo.get('camunda:runscript');
  }
};
