...
function BpmnReplace(bpmnFactory, replace, selection, modeling, eventBus) {
    ...
    function replaceElement(element, target, hints) {
        ...
        //Check if the old type differs from the new type
        if (oldBusinessObject.$type !== newBusinessObject.$type) {
            //check if extensionelements exists in the old object
            if (oldBusinessObject.extensionElements) {
                //Empty the array of formfields
                oldBusinessObject.extensionElements.values[0].fields = [];
            }
        }
